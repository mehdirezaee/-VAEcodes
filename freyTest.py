...
batch_size=100
(vae,loss_step,loss_vec) = train(network_architecture,batch_size=batch_size, training_epochs=60)
...
#print('n_samples: ',n_samples)
#print('first layer weights: ',vae.first_layer_w)
x_sample = frey_images[:,1:batch_size+1]
#print(x_sample[:,1:2].shape)

(x_reconstruct_mean,x_reconstruct_sigma) = vae.reconstruct(np.transpose(x_sample))
plt.figure(figsize=(10,5))
image_number=50;
############## Origianl Images ##########
plt.subplot(131)
plt.title('Original Image')
plt.imshow((x_sample[:,image_number]).reshape(28,20),cmap='gray')
reconst_number=560;
plt.subplot(133)
plt.title('PDF Image')
pdf_image=(1/np.sqrt(2*np.pi))*(np.exp(-np.divide(np.power(np.transpose(x_sample)-x_reconstruct_mean,2),x_reconstruct_sigma)))
plt.imshow(pdf_image[image_number,:].reshape(28,20)*(np.max(frey_images)-np.min(frey_images))+np.min(frey_images),cmap='gray')
#plt.imshow(pdf_image[0,:].reshape(28,20),cmap='gray')
plt.subplot(132)
plt.title('Mean Image')
plt.imshow(x_reconstruct_mean[image_number,:].reshape(28,20),cmap='gray')
plt.tight_layout()
#plt.figure()
#plt.imshow(vae.first_layer_w[:,1:2].reshape(28,20),cmap='gray')
...
'''for i in range(5):
    ...
    #plt.colorbar()
    plt.savefig('figures/Test input'+str(i)+'.png', bbox_inches='tight')
    #plt.subplot(5, 2, 2*i + 2)
    plt.imshow(x_reconstruct[i].reshape(28, 28), vmin=0, vmax=1, cmap="gray")
    plt.title("Reconstruction")
    plt.savefig('figures/Reconstruct' + str(i) + '.png', bbox_inches='tight')
#    plt.colorbar()
plt.tight_layout()
plt.show()
'''
